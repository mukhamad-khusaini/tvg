...
// Helper: format angka jadi string dengan titik ribuan
function cleanNumber(n) {
  if (typeof n === "number") return Math.round(n).toLocaleString("id-ID");
  if (!n) return n;
  const parsed = Number(n);
  return isNaN(parsed) ? n : Math.round(parsed).toLocaleString("id-ID");
}
...
function loadInvoiceFromUploadedExcel(arrayBuffer) {
  ...
  for (let i = 5; i < ordersData.length; i += 5) {
    const row = {
      nights: cleanNumber(ordersData[i]) + " Malam",
      rate: cleanNumber(ordersData[i + 1]),
      isDiscount: ordersData[i + 2],
      totalRate: cleanNumber(ordersData[i + 3]),
      description: ordersData[i + 4],
    };
    // skip placeholder
    if (
...
document.getElementById("generateBtn").addEventListener("click", async () => {
  ...
  const outputName = `Invoice_${data.name}_${data.invNumber}.pdf`;
